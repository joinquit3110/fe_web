    ...
    <ChakraProvider theme={theme}>
      <AuthProvider>
        <AdminProvider>
          <SocketProvider>
            <AppContent />
            {/* Add inline script for standalone notification trigger */}
            <script dangerouslySetInnerHTML={{
              __html: `
                // Create a global utility to trigger test notifications from the console
                window.testNotification = function(options = {}) {
                  const defaultOptions = {
                    id: 'test_' + Date.now(),
                    type: 'success',
                    title: 'TEST NOTIFICATION',
                    message: 'This is a console-triggered test notification',
                    timestamp: new Date(),
                    house: options.house || 'gryffindor',
                    pointsChange: options.points || 10,
                    reason: options.reason || 'Test Notification',
                  };
                  
                  const notification = { ...defaultOptions, ...options };
                  
                  // Option 1: Try the React event system first
                  const testEvent = new CustomEvent('test-notification', {
                    detail: notification
                  });
                  window.dispatchEvent(testEvent);
                  console.log('Test notification dispatched via event system');
                  
                  // Option 2: Create a direct DOM-based notification as fallback
                  try {
                    // Create standalone notification that doesn't rely on React
                    const container = document.createElement('div');
                    container.id = 'fallback-notification-' + notification.id;
                    container.style.position = 'fixed';
                    container.style.top = '20px';
                    container.style.right = '20px';
                    container.style.zIndex = '999999';
                    container.style.backgroundColor = notification.type === 'success' ? 'rgba(0, 128, 0, 0.9)' : 
                    ...
                    if (notification.pointsChange) {
                      ...
                      points.textContent = 'Points: ' + notification.pointsChange;
                    ...
                    console.log('Created fallback DOM notification');
                  } catch (err) {
                ...
                // Add shortcut keys to trigger test notifications
                document.addEventListener('keydown', function(e) {
                  // Alt+T for test notification
                  if (e.altKey && e.code === 'KeyT') {
                    console.log('Test notification shortcut triggered');
                    window.testNotification();
                  }
                  
                  // Alt+P for points test notification
                  if (e.altKey && e.code === 'KeyP') {
                    console.log('Points notification shortcut triggered');
                    window.testNotification({
                ...
                console.log('Notification test utilities loaded. Press Alt+T to test or use window.testNotification() in console');
              `
            }} />
          </SocketProvider>
        </AdminProvider>
